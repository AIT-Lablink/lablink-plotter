...
public class DataSourceSync {
  ...
  /** Logger. */
  private static final Logger logger = LogManager.getLogger( "DataSourceSync" );

  private static final String CLIENT_NAME = "DataSourceSync";

  private static final String GROUP_NAME = "PlotterDemo";

  private static final String SCENARIO_NAME = "PlotterSync";

  private static final String LL_PROP_URI =
      "$LLCONFIG$ait.all.all.llproperties";

  private static final String SYNC_PROP_URI =
      "$LLCONFIG$ait.test.plotter.sync.sync-host.properties";
  ...
  public static void main( String[] args ) throws
    ...
    DataSourceSync app = new DataSourceSync();
    app.setupClient();
  }
  ...
  public void setupClient() throws
    ...
    // Sync consumer.
    DataSourceISyncConsumer sync = new DataSourceISyncConsumer( this );

    // Specify client configuration (no sync host).
    MqttCommInterfaceUtility.addClientProperties( client, clientDescription,
        SCENARIO_NAME, GROUP_NAME, CLIENT_NAME, LL_PROP_URI, SYNC_PROP_URI, sync );
  ...
  class DataSourceISyncConsumer implements ISyncConsumer {

    private DataSourceSync data;

    public DataSourceISyncConsumer( DataSourceSync dss ) {
      this.data = dss;
    }
    ...
    public long go( long currentSimTime, long until, ISyncParameter scs ) {
      ...
      // When in simulation mode, wait a few milliseconds to avoid racing conditions
      // bewteen synchronization points, message delivery between clients and plotting.
